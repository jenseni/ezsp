...
class DistrictWidget extends Controller{
	...
	public function showSelect($city, $curArea = '', $curBusiArea = ''){
		...
		$areaIdList = array();
		$html = '<div class="row">
      			 <div class="col-sm-6"><select id="area" name="area" class="form-control"><option value="">区域</option>';
		foreach($areaList as $area){
			$areaIdList[] = $area['id'];
		...
		if(!empty($busiAreaList)){
			$html .= '<div class="col-sm-6"><select id="busi_area" name="busi_area" class="form-control"><option value="">商区</option>';
			foreach($busiAreaList as $busiArea){
